        ...
        <div class="w-full max-w-md bg-white rounded-lg shadow-md p-8">
            ...
            <form action="{{ route('login.authenticate') }}" method="POST" class="space-y-4">
                ...
                <div class="flex items-center justify-between">
                    <a href="{{ route('register') }}" class="text-sm text-indigo-600 hover:underline">Não tem uma conta? Cadastre-se</a>
                </div>
                <button type="submit" class="w-full py-2 px-4 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 transition">Entrar</button>
            </form>
        </div>
    </div>
</body>
